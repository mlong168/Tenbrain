...
class Paypal_Utils
{
	 ...
	 * @static
	 */
	public static function getURL($fileContextPath_)
	{
...
//        $url = $request->getScheme() . '://' . $request->getHttpHost().":$server_port".$fileContextPath_;
	...
	function PPError($error_msg, $error_no) {
		...
		$redirectURL = self::getURL(substr_replace($php_self, "Error.php", strrpos($php_self, '/') + 1));
